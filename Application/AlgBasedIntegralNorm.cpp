...
Curve AlgBasedIntegralNorm::approximateCurve(const Curve &curve, int degreeApprox) const
{
    ...
    std::vector<QPointF> controlPointsNewCurve(newNumControlPoints);
    ...
    {
        controlPointsNewCurve[1 + i].setX(newPoints[i][0]);
        controlPointsNewCurve[1 + i].setY(newPoints[i][1]);
    }
...
std::vector<std::vector<double>> AlgBasedIntegralNorm::calcNewControlsPointsIntegralsMethod(const Curve &originalCurve, int newNumControlPoints, int newDegreeCurve) const
{
    ...
    std::vector<double> startPointOriginalCurve { originalCurve.getControlPoints()[0].x(), originalCurve.getControlPoints()[0].y() };       // Начальная точка оригинальной кривой
    std::vector<double> endPointOriginalCurve { originalCurve.getControlPoints().back().x(), originalCurve.getControlPoints().back().y() }; // Конечная точка оригинальной кривой
