...
double Metrics::calcCurveCurvature(const std::vector<CurvePoint> &curvePoints)
{
    ...
    {
        secondDeriv.setX(curvePoints[i].derivs[2].x() * curvePoints[i].derivs[2].x());
        secondDeriv.setY(curvePoints[i].derivs[2].y() * curvePoints[i].derivs[2].y());
...
std::pair<CurvePoint, CurvePoint> Metrics::calcPointsHausdorffMetric(const Curve &Curve1, const Curve &Curve2)
{
    ...
    double dist_1 = MathUtils::calcVectorLenght(farthestPoints_1.first.x, farthestPoints_1.first.y, farthestPoints_1.second.x, farthestPoints_1.second.y);
    double dist_2 = MathUtils::calcVectorLenght(farthestPoints_2.first.x, farthestPoints_2.first.y, farthestPoints_2.second.x, farthestPoints_2.second.y);
