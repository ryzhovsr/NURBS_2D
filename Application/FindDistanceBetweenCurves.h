...
private:
    ...
    static double cosBetweenVectors(const CurvePoint &pointNURBS, const QPointF &point);

    // Возвращает точку кривой с наименьшим/наибольшим расстоянием до точки (startingPoint) (Нормаль)
    static CurvePoint findPointNURBS(const Curve &curve, const QPointF &startingPoint, bool nearest);
    static CurvePoint findPointNURBS(const Curve &curve, double x, double y, bool nearest);
    ...
    static CurvePoint comparePoints(const std::vector<CurvePoint> &points, const std::vector<double> &cosines, const QPointF& startingPoint, bool nearest);
};
